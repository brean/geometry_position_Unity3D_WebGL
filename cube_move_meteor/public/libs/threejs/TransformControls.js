...
( function () {
	...
	THREE.TransformGizmoTranslate = function (usedAxis) {
		...
		if (usedAxis.indexOf('X') >= 0 && usedAxis.indexOf('Y') >= 0 && usedAxis.indexOf('Z') >= 0) {
			this.handleGizmos['XYZ'] = [
				[ new THREE.Mesh( new THREE.OctahedronGeometry( 0.1, 0 ), new GizmoMaterial( { color: 0xffffff, opacity: 0.25 } ) ), [ 0, 0, 0 ], [ 0, 0, 0 ] ]
			];
		}
		if (usedAxis.indexOf('X') >= 0 && usedAxis.indexOf('Y') >= 0) {
			this.handleGizmos['XY'] = [
				[ new THREE.Mesh( new THREE.PlaneBufferGeometry( 0.29, 0.29 ), new GizmoMaterial( { color: 0xffff00, opacity: 0.25 } ) ), [ 0.15, 0.15, 0 ] ]
			];
		}
		if (usedAxis.indexOf('Y') >= 0 && usedAxis.indexOf('Z') >= 0) {
			this.handleGizmos['YZ'] = [
				[ new THREE.Mesh( new THREE.PlaneBufferGeometry( 0.29, 0.29 ), new GizmoMaterial( { color: 0x00ffff, opacity: 0.25 } ) ), [ 0, 0.15, 0.15 ], [ 0, Math.PI / 2, 0 ] ]
			];
		}
		if (usedAxis.indexOf('X') >= 0 && usedAxis.indexOf('Z') >= 0) {
	...
	THREE.TransformControls = function ( camera, domElement, usedAxis ) {
		...
		var _gizmo = {
			...
			"translate": new THREE.TransformGizmoTranslate(usedAxis),
